...
function doRectanglesOverlap(rect1, rect2) {
  const distX = rect1.left + rect1.width;
  const distY = rect1.top + rect1.height;
  return distX > rect2.left && distY > rect2.top;
}
...
function isInsideCircle(circle, point) {
  const x1 = point.x - circle.center.x;
  const y1 = point.y - circle.center.y;
  const s = Math.sqrt(x1 ** 2 + y1 ** 2);
...
function findFirstSingleChar(str) {
  ...
  return chars.find((i) => str.indexOf(i) === str.lastIndexOf(i));
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((item) => {
    if (stack.length > 0 && stack[stack.length - 1] === dictionary[item]) {
      stack.pop();
    } else {
      stack.push(item);
    }
